...
if (usePostgres && postgresUrl) {
  try {
    console.log("Creating database pool for:", postgresUrl.split('@')[1] || "Postgres");
    pool = new Pool({
...
const getSqliteDb = async () => {
  ...
  sqliteDb = new Database(path.join(__dirname, "..", "lit_store.db"));
  return sqliteDb;
};
...
app.use(async (req, res, next) => {
  // Skip DB init for health check to allow diagnostics
  if (req.path === '/api/health') return next();

  try {
    if (!dbInitialized) {
      if (dbInitError) {
        throw new Error(`La base de datos falló anteriormente: ${dbInitError}`);
      }
    ...
    res.status(500).json({ 
      ...
      hint: "Verifica tu POSTGRES_URL en Vercel."
    });
  }
});
