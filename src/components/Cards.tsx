  ...
  return (
    <div className="group relative bg-white border border-gray-100 overflow-hidden transition-all hover:shadow-xl hover:border-lit-green/20">
      ...
      <div className="p-6">
        ...
        <p className="text-xs text-lit-purple/40 mb-6 line-clamp-1">{product.focus}</p>
...
export const PackCard: React.FC<{ pack: Pack }> = ({ pack }) => {
  return (
    <div className="group relative bg-white border border-gray-100 text-lit-purple overflow-hidden transition-all hover:shadow-2xl">
      <div className="relative aspect-[16/9] overflow-hidden bg-white">
        <img 
          src={pack.image} 
          alt={pack.name} 
          className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105 opacity-80 grayscale"
          referrerPolicy="no-referrer"
        />
      ...
      <div className="p-8 relative">
        ...
        <p className="text-sm text-lit-purple/50 mb-8 line-clamp-2 font-light leading-relaxed">{pack.description}</p>
        
        <div className="flex items-center justify-between">
          <span className="text-2xl font-bold text-lit-green">${pack.price.toFixed(2)}</span>
          <Link to={`/pack/${pack.id}`} className="btn-primary bg-lit-purple text-white hover:bg-lit-green py-2 px-6">
            Ver Pack
          </Link>
        </div>
      </div>
